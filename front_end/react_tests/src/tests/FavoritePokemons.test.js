...
describe('Requisito 3', () => {
  it('se exibe No favorite pokémons quando não há favoritos', () => {
    renderWithRouter(<FavoritePokemons pokemons={ [] } />);
    const noFavorite = screen.getByText(/no favorite pokemon found/i);
    expect(noFavorite).toBeInTheDocument();
  });
  it('se é exibido todos os cards de pokémons favoritos', () => {
    const favorites = [
      {
        id: 4,
        name: 'Charmander',
    ...
    const charmanderTitle = screen.getByText(/charmander/i);
    const charmanderImg = screen.getByRole('img', {
      name: /charmander sprite/i,
    });
    const caterpieTitle = screen.getByText(/caterpie/i);
    ...
    expect(caterpieTitle).toBeInTheDocument();
