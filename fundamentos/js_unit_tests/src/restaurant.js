...
const orderFromMenu = (request) => consumed.push(request);
  ...
  const items = {
    ...
    pay: () => {
      for (let index = 0; index < consumed.length; index += 1) {
        if (objGiven.food[consumed[index]] !== undefined) {
          sumOrders += objGiven.food[consumed[index]];
        } else {
          sumOrders += objGiven.drink[consumed[index]];
        }
      }
    const totalWithTip = sumOrders * 1.1;
    return totalWithTip;
    },
  };
  return items;
